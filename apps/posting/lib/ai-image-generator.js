...
export async function generateAiDrawing({
  ...
  if (imageModelManager?.activeModelId && imageModelManager.activeModelId !== 'pollinations') {
    await imageModelManager.generate({ prompt: enhancedPrompt, style, outputPath: filePath });
    const active = (await imageModelManager.list()).find((model) => model.id === imageModelManager.activeModelId);
    return { id:`local-art-${randomUUID().slice(0,8)}`,title:prompt.slice(0,60),filePath,previewUrl:`/generated-images/thumb/${filename}`,downloadUrl:`/generated-images/${filename}`,thumbnailUrl:`/generated-images/thumb/${filename}`,pageUrl:'',author:active?.name||'로컬 이미지 AI',license:`${active?.name||'로컬 이미지 AI'} 생성`,licenseUrl:'',afterHeading,caption:`${active?.name||'로컬 AI'}: ${prompt.slice(0,60)}`,isAiGenerated:true,style,autoSelected:true,imageModelId:active?.id||imageModelManager.activeModelId };
  }

  // Smart ComfyUI Check: If ComfyUI is running on port 8188, prefer it for fast 1~2s high-quality local generation!
  const comfyAvailable = await isComfyUiRunning().catch(() => false);
  if (comfyAvailable) {
    try {
      await generateWithComfyUi({ prompt: enhancedPrompt, width, height, seed, outputPath: filePath });
      return {
        id: `comfy-art-${randomUUID().slice(0, 8)}`,
        ...
        pageUrl: '',
        author: 'ComfyUI Z-Image-Turbo (로컬 RTX 5080)',
        license: 'ComfyUI 로컬 생성',
        licenseUrl: '',
        afterHeading,
        caption: `ComfyUI Z-Image-Turbo: ${prompt.slice(0, 60)}`,
        isAiGenerated: true,
        style,
        autoSelected: true,
        imageModelId: 'comfyui-z-image-turbo'
      };
    } catch (comfyErr) {
      console.warn('[ImageGen] ComfyUI generation failed, falling back to online FLUX:', comfyErr.message);
    }
  }
...
export async function generateAiDrawingsForPost(post, outputDir, { style = 'photorealistic', fetchImpl = fetch, imageModelManager = null } = {}) {
