...
export class EngagementHistoryStore {
  ...
  normalizeUrlKey(url) {
    ...
    try { const parsed = new URL(text.startsWith('http') ? text : `https://${text}`); const logNo = parsed.searchParams.get('logNo') || parsed.pathname.match(/\/(\d{8,15})(?:\/|$)/)?.[1]; const blogId = parsed.searchParams.get('blogId') || parsed.pathname.match(/\/([^/]+)\/(?:\d{8,15})(?:\/|$)/)?.[1]; if (blogId && logNo) return `${blogId.toLowerCase()}/${logNo}`; return `${parsed.hostname.replace(/^m\./, '').toLowerCase()}${parsed.pathname.replace(/\/+$/, '').toLowerCase()}`; } catch { return text.toLowerCase().replace(/^https?:\/\//, '').replace(/^m\./, '').replace(/[?#].*$/, '').replace(/\/+$/, ''); }
  }
