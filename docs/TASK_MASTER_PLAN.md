...
상용 과금 인프라와 보안 체계를 조기에 확립하기 위해 **[클라이언트 핵심 기능] ➔ [중앙 인증 & 라이선스 결제 서버] ➔ [Gemma 4 E2B AI 파인튜닝] ➔ [DRM 보안 패키징] ➔ [통합 검증 및 런칭]** 순으로 진행합니다.
...
graph TD
    ...
    subgraph "Phase 2 [우선순위 격상]: 중앙 라이선스 & 결제 인증 서버 구축"
        T2_1[Task 2.1: 중앙 인증 API 서버 개발 - 회원/만료일] --> T2_3[Task 2.3: 결제 웹훅 연동 - 토스/스토어]
        T2_2[Task 2.2: 1인 1PC HWID 기기 지문 바인딩] --> T2_4[Task 2.4: 클라이언트 10분 주기 하트비트 연동]
        T2_3 --> T2_4
    end

    subgraph "Phase 3: Gemma 4 E2B AI 파인튜닝 (온디바이스 경량화)"
        T3_1[Task 3.1: 네이버 10대 카테고리 10만건 데이터셋 수집] --> T3_2[Task 3.2: 노이즈 제거 & 네거티브 페널티 라벨링]
        T3_2 --> T3_3[Task 3.3: Unsloth LoRA SFT 및 DPO 학습]
        T3_3 --> T3_4[Task 3.4: GGUF Q4_K_M 양자화 & llama.cpp 서빙 연동]
    end
...
### 🔐 Phase 2 [우선순위 격상]: 중앙 인증 & 라이선스 결제 서버 구축
> **목표**: 결제한 유료 회원만 프로그램을 쓸 수 있도록 1인 1PC 하드웨어 바인딩(HWID)과 결제 자동 연동 인프라를 AI 모델보다 먼저 완성하여 상용 과금 체계 조기 확보.

#### [Task 2.1] 중앙 인증 API 서버 개발 (Auth Server)
* **수행 내용**:
...
#### [Task 2.2] 1인 1PC HWID(하드웨어 지문) 바인딩 및 중복 차단
* **수행 내용**:
...
#### [Task 2.4] 클라이언트 라이선스 인증 화면 & 하트비트 연동
* **수행 내용**:
...
#### [Task 3.2] 노이즈 정제 및 네거티브/포지티브 라벨링
* **수행 내용**:
...
#### [Task 3.3] Unsloth LoRA SFT 및 DPO 파인튜닝 실행
* **수행 내용**:
...
#### [Task 3.4] GGUF Q4_K_M 양자화 및 온디바이스 서빙 연동
* **수행 내용**:
...
| **2주차** | **중앙 라이선스 & 결제 서버 구축** | Task 2.1 ~ 2.4 | 1인 1PC HWID 락 + 결제 웹훅 연동 + 만료일 자동 관리 (상용 과금 인프라 조기 확보) |
| **3주차** | **Gemma 4 E2B AI 파인튜닝** | Task 3.1 ~ 3.4 | 10만건 데이터셋 학습 + Q4 양자화 모델(1.3GB) + 0.5초 로컬 추론 |
...
2. **[Step 2] Phase 2 중앙 라이선스 & 결제 서버 구축 착수**:
   * 사용자 계정 DB, 1인 1PC HWID 기기 지문 검증 로직, 토스페이먼츠 웹훅 연동 착수.
